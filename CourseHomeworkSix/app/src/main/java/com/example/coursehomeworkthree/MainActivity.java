...
import static com.example.coursehomeworkthree.DBHelper.BIRTH_COL;
import static com.example.coursehomeworkthree.DBHelper.EMAIL_COL;
import static com.example.coursehomeworkthree.DBHelper.IMG_COL;
import static com.example.coursehomeworkthree.DBHelper.NAME_COL;
...
public class MainActivity extends AppCompatActivity implements UserRecyclerViewAdapter.UserClickListener {
    ...
    private void saveData(){
        ...
        for(UserClass user:users){
            cv.clear();
            cv.put(NAME_COL, user.getUsername());
            cv.put(BIRTH_COL, user.getDateOfBirth());
            cv.put(EMAIL_COL, user.getEmail());
            cv.put(IMG_COL, user.getImg_url());
    ...
    private void showData(){
        ...
        if (c.moveToFirst()) {
            ...
            int nameColIndex = c.getColumnIndex(NAME_COL);
            int emailColIndex = c.getColumnIndex(EMAIL_COL);
            int birthColIndex = c.getColumnIndex(BIRTH_COL);
            int imgColIndex = c.getColumnIndex(IMG_COL);
            ...
            do {
                Log.d(LOG_TAG,
                        "ID = " + c.getInt(idColIndex) +
                                ...
                                ", date of birth = " + c.getString(birthColIndex) +
                                ", img url = " + c.getString(imgColIndex));
            } while (c.moveToNext());
        } else
